...
public class UpdateDiaryEntryAction extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d("UpdateDiaryEntryAction", "id: "+id);

        DiaryEntry oldEntry = entriesService.getEntryById(UUID.fromString(id));

        Log.d("UpdateDiaryEntryAction", "entry date: "+oldEntry.getDate());
        titleEditText.setText(oldEntry.getTitle());
        entryTextEditText.setText(oldEntry.getEntryText());
        seekBar.setProgress(oldEntry.getMood());
        formattedDate = oldEntry.getDate();
        EditText dateEditText = findViewById(R.id.dateEditText);
        dateEditText.setText(formattedDate);

        byte[] imageBytes = oldEntry.getImageBytes();
        if (imageBytes != null) {
            ImageView imageView = findViewById(R.id.imageView);
            Bitmap capturedImage = BitmapFactory.decodeByteArray(imageBytes, 0, imageBytes.length);
            imageView.setImageBitmap(capturedImage);
        }
        latitude = oldEntry.getLatLong().latitude;
        longitude = oldEntry.getLatLong().longitude;
        ...
        cameraButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (camera.hasCameraPermission()) {
                    ...
                    Toast.makeText(UpdateDiaryEntryAction.this, "Camera app is not available.", Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    public void saveDiaryEntry(DiaryEntry oldEntry) {
        // disable done button
        ...
        // Update DiaryEntry object
        DiaryEntry newEntry = oldEntry.updateDiaryEntry(formattedDate, title, entryText, feeling);
        ...
        Disposable subscribe = geoCoder.getAddress(latlong)
                ...
                .subscribe(address -> {
                    ...
                    entriesService.updateEntry(newEntry);
                    entriesService.getDiaryListAdapter().notifyDataSetChanged();
